...
var WebSocketServer = class extends import_events.default {
  ...
  constructor({ server: server2, realm, config }) {
    ...
    this.socketServer.on(
      "connection",
      (socket, req) => this._onSocketConnection(socket, req)
    );
  ...
  _onSocketConnection(socket, req) {
    ...
    if (!id || !token || !key) {
      return this._sendErrorAndClose(socket, "No id, token, or key supplied to websocket server" /* INVALID_WS_PARAMETERS */);
    }
  ...
  _registerClient({
    ...
    if (clientsCount >= this.config.concurrent_limit) {
      return this._sendErrorAndClose(socket, "Server has reached its concurrent user limit" /* CONNECTION_LIMIT_EXCEED */);
    }
...
var public_default = ({ config, realm }) => {
  ...
  app.get("/network", (_, res) => {
    if (config.allow_discovery) {
      ...
      const clients = realm.getClients();
      clients.forEach((value, key) => {
        if (requestIp !== ipToIpv4(value.getSocket()._socket.remoteAddress))
          clients.delete(key);
      });
      const fixedClients = [];
      for (const [key, client] of clients.entries()) {
...
var createInstance = ({
  ...
  const customConfig = { ...config, path: import_path.default.posix.join(app.path(), options.path, "/") };
